...
class Model
{
    ...
    public function __construct()
    {
        try {
            ...
            throw new Exception('MySQL 数据库链接失败','2002');
        }
    ...
    public function sql()
    {
        echo sprintf("SELECT %s FROM `%s` %s %s", $this->field, $this->table,$this->join,$this->filter);
    }
    ...
    {
        $sql = sprintf("UPDATE `%s` SET %s %s",$this->table,$field.'='.$field.'+'.$step,$this->filter);
        $sth = $this->dbHandle->prepare($sql);
        $sth->execute();
        $this->destroy();
        return $sth->rowCount();
    ...
    public function MAX($field)
    {
        ...
        $sql = sprintf("SELECT MAX(%s) as max  FROM `%s` %s %s",$field,$this->table,$this->join,$this->filter);
        $sth = $this->dbHandle->prepare($sql);
        $sth->execute();
        $this->destroy();
        $data = $sth->fetch();
    ...
    public function MIN($field)
    {
        ...
        $sql = sprintf("SELECT MIN(%s) as min  FROM `%s` %s %s",$field,$this->table,$this->join,$this->filter);
        $sth = $this->dbHandle->prepare($sql);
        $sth->execute();
        $this->destroy();
        $data = $sth->fetch();
